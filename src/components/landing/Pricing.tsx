    ...
    <section id="cenik" className="max-w-7xl mx-auto px-6 py-20">
      ...
      <div className="mb-12 flex justify-center">
        <label className="relative flex items-center p-1 bg-[#e2e8f0] rounded-full cursor-pointer w-[16rem] shadow-[inset_0_2px_4px_rgba(0,0,0,0.06),0_1px_1px_rgba(255,255,255,1)] border border-slate-300">
          ...
          <span
            className={`relative w-1/2 flex items-center justify-center gap-1.5 text-center text-sm font-normal transition-colors duration-300 py-2.5 z-10 ${
            ...
            <div className="flex justify-between items-start pt-2">
              <div>
                <div className="flex items-baseline text-slate-800">
                  ...
                  <span className="text-5xl font-normal tracking-tight ml-0.5">{displayPrice}</span>
            ...
            <div className="flex flex-col gap-6 flex-grow">
              {sections.map((section, idx) => (
                <div key={idx} className="flex flex-col gap-3">
                  <h4 className="text-xs font-medium text-blue-500 uppercase tracking-widest">
                    {section.title}
                  </h4>
                  <ul className="flex flex-col gap-y-3">
                    {section.items.map((item, itemIdx) => (
                      <li key={itemIdx} className="flex items-start gap-3 text-sm font-normal text-slate-700">
